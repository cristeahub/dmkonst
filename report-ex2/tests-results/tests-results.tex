...
\begin{figure}[H]
  \begin{code}
    ...
    beq $2, $0, 3     # Should always have to predict this
    sw $2, 2($0)
...
The theoretical clock frequency of the processor is 62.38 MHz.
This is far above the 24 MHz production simulation frequency, but lower than the multi-cycle processor we previously implemented.
...
If this can be shortened, the clock frequency would increase.
...
When comparing execution time of programs run on this pipelined processor with the multi-cycle processor we implemented in exercise 1, we see that even though the clock frequency is slightly reduced, the overall performance is increased significantly. See table \ref{table:execution_time} for exact timings.
