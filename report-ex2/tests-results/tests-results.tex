...
\begin{table}[h!]
    ...
    Hazard detection  & Confirm that the pipeline is not stalled when an r-type instruction follows a load, but there's no overlap in registers & \checkmark Passed \\ \cline{2-2}
                      & Check that the pipeline is only stalled when a memory read is in the execute stage. & \\\cline{2-2}
                      & Check that the pipeline is not stalled when a store instruction is dependent on the load. & \\ \hline
